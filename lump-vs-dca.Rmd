...
```{r, results='hide', cache=TRUE, cache.extra = Sys.Date()}
...
* On the days **DCA** wins, it wins over **LS** by $`r sprintf('%i', dca_mean_win_by)` on average
* On the days **LS** wins, it wins over **DCA** by $`r sprintf('%i', lump_mean_win_by)` on average

So, not only does **DCA** win only in `r dca_wins_p`% of cases, its performance is on average, `r round(lump_mean_win_by/dca_mean_win_by,2)` times **worse** even when it wins.
